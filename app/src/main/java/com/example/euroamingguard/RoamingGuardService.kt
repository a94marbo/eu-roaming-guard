...
class RoamingGuardService : Service() {
    ...
    override fun onCreate() {
        ...
        startForeground(NOTIF_ID, createNotification("Övervakar nätverksstatus..."))
    ...
    private fun evaluateNetwork() {
        ...
        if (isAllowed) {
            ...
            updateNotification("Ansluten till godkänt nät (MCC $mcc). Roaming AKTIVERAD.")
        } else {
            RoamingController.setRoamingEnabled(this, false)
            updateNotification("Icke-godkänt nät (MCC $mcc). Roaming INAKTIVERAD.")
        }
    }
